    ...
    public static class DBClass
    {
        ...
        */
        /*        
                ...
                private static void DB_Update_Record(string tableName, string primaryKeyColumnName, List<string> element_to_update)
                {
                    string sSQL = "SELECT * FROM " + tableName + " WHERE [" + primaryKeyColumnName[0] + "] = '" + element_to_update[0] + "'";
                    DataTable tbl = Get_DataTable(sSQL);
                    if (tbl.Rows.Count > 0)
                    {
                        for (int i = 1; i < element_to_update.Count; i++)
                        {
                            string sql_Update = "UPDATE " + tableName + " SET [" + columnNames[i] + "] = '" + element_to_update[i] + "' WHERE [" + primaryKeyColumnName + "] = '" + element_to_update[0] + "'";
                            Execute_SQL(sql_Update);
                        }
                    }
                }

                private static void DB_Remove_Record(string tableName, string id)
                {
                    string sSQL = "SELECT * FROM " + tableName + " WHERE [" + primaryKeyColumnName + "] = '" + id + "'";
                    DataTable tbl = Get_DataTable(sSQL);
                    if (tbl.Rows.Count > 0)
                    {
                        string sql_Remove = "DELETE FROM " + tableName + " WHERE [" + primaryKeyColumnName + "] = '" + id + "'";
